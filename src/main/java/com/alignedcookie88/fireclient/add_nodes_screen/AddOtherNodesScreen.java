...
public class AddOtherNodesScreen extends AddNodesScreen {
    ...
    protected void init() {
        ...
        addButton("Add Events Node", button -> {
            addNode("Events", "event");
        }, x, y, Items.DEEPSLATE_GOLD_ORE);

        y += y_change;
        addButton("Add Events Dev Node", button -> {
            addNode("Events Dev", "dev-events");
        }, x, y, Items.DEEPSLATE_REDSTONE_ORE);

        y += y_change;
        if (FabricLoader.getInstance().isModLoaded("viafabric") || FabricLoader.getInstance().isModLoaded("viafabricplus")) {
            addButton("Add DiamondFire Education", button -> {
        ...
        y += y_change;
        addButton("Add Dev 3 - Forest", button -> {
            addNumericalDevNode(3, "Forest");
        }, x, y, Items.REPEATING_COMMAND_BLOCK);

        y += y_change;
        addButton("Add Build Node", button -> {
            addNode("Build", "build");
